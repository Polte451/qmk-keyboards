...
// file scope, before process_record_user
// Key Combo
static bool lctl_held = false;
static bool lsft_held = false;
static bool rctl_held = false;
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        // Key Combo
        // Left Ctrl + Right Ctrl = Caps Lock
        // Right Control (Caps Lock) + Space = Toggle Layer 5 - IJKL to arrow keys
        // Left Shift + Space = Toggle Layer 6 - IJKL to mouse
        case KC_LCTL:
        ...
        case KC_LSFT:
            ...
            break;
        case KC_RCTL:
            rctl_held = record->event.pressed;
            if (record->event.pressed && lctl_held) {
                tap_code(KC_CAPS);
                return false;
            }
            break;
        case KC_SPC:
            if (record->event.pressed && rctl_held) {
                layer_invert(5);
                return false;
            }
